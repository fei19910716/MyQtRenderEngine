...
void PopupTriangleWidget::paintEvent(QPaintEvent *e)
{
    ...
    if (m_orien == PWB::Horizontal) {
        ...
        points << QPointF(0, 0) << QPointF(width(), 0) << QPointF(width()/2, height()); // 三角形的三个点
        QPolygonF polyF(points);
        path.addPolygon(polyF);
        painter.drawPath(path);
    }
      ...
      m_shadowWidth(4)
{
    ...
    m_pTriangleWidget->setStyleSheet("border: 4px;");
    m_pMainWidget->setMinimumSize(270, 210);
    ...
    if (m_orien == PWB::Horizontal) {
        ...
        m_pVLayout->addSpacing(m_shadowWidth + 12); // 增加上方的padding，这样显示出上方的三角形
        m_pVLayout->addWidget(m_pMainWidget);
        // m_pVLayout->setSpacing(0);
        m_pVLayout->addSpacing(m_shadowWidth + 12); // 增加下方的padding，这样显示出下方的三角形
        m_pVLayout->setContentsMargins(m_shadowWidth, m_shadowWidth, m_shadowWidth, m_shadowWidth);
        // m_pTriangleWidget->move(m_pMainWidget->width()/2-10 + m_shadowWidth, m_shadowWidth + 5); // 控制三角形显示的位置，上方显示
        m_pTriangleWidget->move(m_pMainWidget->width()/2-10 + m_shadowWidth, m_shadowWidth + 5 + m_pMainWidget->height() + 10); // 控制三角形显示的位置，下方显示
#if 0
        m_pVLayout = new QVBoxLayout(this);
    ...
    connect(this, &PopupWidgetButton::ButtonClicked, [=]() {
        if (m_pMainWidget->isHidden()) {
            ...
            if (m_orien == PWB::Horizontal) {
                ...
                pos.setY(m_pButton->mapToGlobal(QPoint(0, 0)).y() - m_pMainWidget->height()); // 控制弹出窗口的位置
            }
